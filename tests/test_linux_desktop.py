...
from whispr_flow_linux.linux_desktop import (
    autostart_dir,
...
class LinuxDesktopTest(unittest.TestCase):
    ...
    def test_autostart_entry_runs_server_without_browser(self):
        entry = build_autostart_entry(["whispr-flow", "desktop", "--no-browser"])
        self.assertIn("Exec=whispr-flow desktop --no-browser", entry)
    ...
    def test_install_and_uninstall_autostart_entry(self):
        with tempfile.TemporaryDirectory() as directory:
            with mock.patch.dict(os.environ, {"XDG_CONFIG_HOME": directory}):
                ...
                path = install_autostart_entry(["whispr-flow", "desktop", "--no-browser"])
                self.assertEqual(path, Path(directory) / "autostart" / "whispr-flow-linux.desktop")
