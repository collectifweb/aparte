...
from whispr_flow_linux import session


class ToggleSessionTest(unittest.TestCase):
    def test_runtime_dir_can_be_overridden(self):
        with tempfile.TemporaryDirectory() as directory:
            with mock.patch.dict(os.environ, {"WHISPR_RUNTIME_DIR": directory}):
            ...
            try:
                with mock.patch.dict(
                    ...
                    with mock.patch("tempfile.gettempdir", return_value=temp_dir):
                        self.assertEqual(session.get_runtime_dir(), Path(temp_dir) / f"whispr-flow-{os.getuid()}")
            finally:
