...
def _save_until_killed(source, connection):
    ...
    def partial_copy(_source, target):
        ...
        target.flush()
        directory = next((Path(source).parent / "recovery").iterdir())
...
def _save_before_first_yield(source, connection):
    """Pause during creation once audio is copied, before save_and_claim yields."""
    copy = recovery.shutil.copyfileobj
    def pause_after_copy(stream, target):
        copy(stream, target)
        target.flush()
        connection.send(str(next((Path(source).parent / "recovery").iterdir())))
        connection.recv()
    with mock.patch.object(recovery.shutil, "copyfileobj", side_effect=pause_after_copy):
        with recovery.save_and_claim(Path(source)):
            pass
...
class RecoveryTest(unittest.TestCase):
    ...
    def test_new_capture_stays_claimed_until_caller_finishes(self):
        with recovery.save_and_claim(self.source, "raw text") as item:
            self.assertEqual(item.raw_text, "raw text")
            self.assertEqual(item.audio_path.read_bytes(), self.source.read_bytes())
            self.assertTrue(recovery.entries()[0]["busy"])
            with self.assertRaises(recovery.RecoveryBusy):
                recovery.discard(item.id)
            self.assertEqual(recovery.sweep(now=item.expires_at + 1), 0)
        self.assertFalse(recovery.entries()[0]["busy"])
        recovery.discard(item.id)
        self.assertEqual(recovery.entries(), [])

    def test_new_capture_survives_exception_after_yield(self):
    ...
    def test_new_capture_creation_failure_cleans_copy_but_keeps_source(self):
        with mock.patch.object(recovery.shutil, "copyfileobj", side_effect=OSError("disk full")):
            with self.assertRaises(OSError):
                with recovery.save_and_claim(self.source):
                    self.fail("incomplete audio yielded")
        self.assertTrue(self.source.exists())
    ...
    def test_another_process_cannot_delete_during_creation_before_yield(self):
        process, name = self._child(_save_before_first_yield, str(self.source))
        identifier = Path(name).name
        self.assertTrue(recovery.entries()[0]["busy"])
        with self.assertRaises(recovery.RecoveryBusy):
            recovery.discard(identifier)
        process.kill()
